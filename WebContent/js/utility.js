...
sap.ui.model.json.JSONModel.extend("utility", {
	                ...
	                /*                   if (!table.nodeType) 
	                            ...
	                            /* if (navigator.appName == 'Microsoft Internet Explorer' || navigator.appName == 'Netscape')
	            	             ...
	            	             if ((navigator.appName == 'Microsoft Internet Explorer') || (!!navigator.userAgent.match(/Trident.*rv[ :]*11\./)))
	            	           	    ...
	            	           	    {
	            	     	        	var b64Data = base64(format(template, ctx));
	            	     	        	...
	            	     	        	var blob = new Blob([byteArray], {type: 'application/vnd.ms-excel', file: 'downloadismail.xls'});
	            	     	        	var blobUrl = URL.createObjectURL(blob);
	            	     	        	//window.open(blobUrl);		// MACHANAEXCELISSUE-
	...
	doOnlineRequest: function (serviceUrl, odataUrlToCall, successfun, errorfunc){
	...
	getOnlineFileDataSuccess: function(resultdata, response){
	   ...
	   try{
			if(resultdata != undefined){
				oUtil.openFilesClientSide(resultdata.File1,resultdata.Path);
			}
	   }catch(e){
		   busyDialog.close();
